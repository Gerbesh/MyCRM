...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const addContractorUrl = root.getAttribute('data-add-contractor-url') || '';
    const editUrlTemplate =
      root.getAttribute('data-edit-contractor-url-template') || '';
    const deleteUrlTemplate =
      root.getAttribute('data-delete-contractor-url-template') || '';

    const tableView = qs('#contractors-table');
    const cardsView = qs('#contractors-cards');
    const tableSkeleton = qs('#contractors-table-skeleton');
    const cardsSkeleton = qs('#contractors-cards-skeleton');
    ...
    if (modal) {
      modal.addEventListener('show.bs.modal', function (event) {
        ...
        const name = button.getAttribute('data-contractor-name');
        ...
        fetch(`/api/v1/requests/by_contractor/${contractorId}`)
          .then((response) => {
            if (!response.ok) {
          ...
          .then((data) => {
            ...
            data.forEach((req) => {
              ...
              const contractorsHtml = (req.contractors || [])
                .map(
                  (c) => `<span class="badge bg-primary me-1">${c.name}</span>`
                )
                .join('');
    ...
    const addContractorBtn = qs('#add-contractor-btn');
    const addContractorModalEl = qs('#addContractorModal');
    const addContractorForm = qs('#add-contractor-form');
    const contractorNameInput = qs('#contractor-name');
    const contractorNameError = qs('#contractor-name-error');
    if (
      addContractorBtn &&
      addContractorModalEl &&
      addContractorForm &&
      contractorNameInput &&
      contractorNameError
    ) {
      // eslint-disable-next-line no-undef
      const addContractorModal = new bootstrap.Modal(addContractorModalEl);
      addContractorBtn.addEventListener('click', function (e) {
        e.preventDefault();
        addContractorForm.reset();
        contractorNameInput.classList.remove('is-invalid');
        contractorNameError.textContent = '';
        addContractorModal.show();
      });
      addContractorForm.addEventListener('submit', function (e) {
        e.preventDefault();
        const name = contractorNameInput.value.trim();
        if (!name) {
          contractorNameInput.classList.add('is-invalid');
          contractorNameError.textContent = 'Название обязательно';
          return;
        }
        ...
        fetch(addContractorUrl || '/contractor/add', {
          method: 'POST',
          body: formData,
        })
          .then(async (r) => {
          ...
          .then((data) => {
            if (data.error) {
              contractorNameInput.classList.add('is-invalid');
              contractorNameError.textContent = data.error;
            } else {
              addContractorModal.hide();
              appendContractorRow(data, {
                isAdmin,
                editUrlTemplate,
                deleteUrlTemplate,
              });
          ...
          .catch((error) => {
            ...
            contractorNameInput.classList.add('is-invalid');
            contractorNameError.textContent = 'Сервер недоступен';
          });
      });
    }

    function appendContractorRow(data, ctx) {
      const { isAdmin, editUrlTemplate, deleteUrlTemplate } = ctx;
      const countEl = qs('#contractors-count');
      if (countEl)
        countEl.textContent = String(parseInt(countEl.textContent, 10) + 1);

      const tableBody = qs('#contractors-table tbody');
      if (tableBody) {
        ...
        if (isAdmin) {
          ...
          adminActions = `<a href="${editUrl}" class="btn btn-sm btn-primary">Редактировать</a>
            <form method="POST" action="${deleteUrl}" style="display:inline;">
              ...
              <button type="submit" class="btn btn-sm btn-danger" data-confirm="Вы уверены, что хотите удалить подрядчика? Это действие нельзя отменить.">Удалить</button>
            </form>`;
        }
        row.innerHTML =
          `<td>${data.name}</td><td>—</td><td>—</td><td>—</td><td>—</td>
          <td><button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#requestsModal" data-contractor-id="${data.id}" data-contractor-name="${data.name}">Заявок: 0</button></td>` +
          (isAdmin ? `<td>${adminActions}</td>` : '');
        tableBody.prepend(row);
      }

      const cards = qs('#contractors-cards');
      if (cards) {
        ...
        card.innerHTML = `<div class="card-body">
            ...
            <p><strong>Заявки:</strong> <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#requestsModal" data-contractor-id="${
              data.id
            }" data-contractor-name="${data.name}">0</button></p>
            ${isAdmin ? adminBlock : ''}
          </div>`;
        cards.prepend(card);
      }
